...
export const TextureLoader = (pixiRenderer: PIXI.Renderer) => {
  ...
  const backboard = pixiRenderer.generateTexture(gfx, PIXI.SCALE_MODES.NEAREST, devicePixelRatio, bounds)
  ...
  const inCircle = pixiRenderer.generateTexture(gfx, PIXI.SCALE_MODES.NEAREST, devicePixelRatio, inBounds)
  ...
  const outCircle = pixiRenderer.generateTexture(gfx, PIXI.SCALE_MODES.NEAREST, devicePixelRatio, outBounds)
  PIXI.Texture.addToCache(outCircle, 'outCircle')


  //SAMPLE CHANGE LATER
  const itemimg = items.getItemIcon('item-electromagnetic-control-rod', 64)
  ...
  const machineimg = buildings.getBuildingIcon('building-assembler-mk1', 256)
