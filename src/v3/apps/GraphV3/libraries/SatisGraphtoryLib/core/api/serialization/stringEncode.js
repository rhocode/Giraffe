...
export function ord(chr) { return chr.charCodeAt(0); }
...
export function isASCII(str) { return isASCIIRE.test(str); }
...
export function view8(buf, start, len) {
  ...
  if (!start && !len && buf instanceof Uint8Array && !buf.copy)
    return buf;
  start = start >>> 0;
  if (len === undefined)
    len = buf.byteLength - start;
  return new Uint8Array(buf.buffer, buf.byteOffset + start, len);
}

export function utf8bytes(str, allowAsyncChars) {
  var l = str.length, i = 0, u = 0, c, a = -1, asy = +!!allowAsyncChars;
  ...
  for (; i < l;) {
    ...
    a = (c & 0xE0) !== 0xC0
            ? (c & 0xF0) !== 0xE0 ? (c & 0xF8) !== 0xF0 ? false : 3 : 2
            : 1;
    if (!a)
